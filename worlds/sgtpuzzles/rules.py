...
def set_rules(multiworld: MultiWorld, player: int, puzzles: list[str]):
    for i in range(max_puzzles):
        if i < len(puzzles):
            ...
            set_rule(multiworld.get_location(f"Puzzle {i+1} Reward 2", player), lambda state, i=i: state.has(f"Puzzle {i+1}", player, 1))


# Sets rules on completion condition
def set_completion_rules(multiworld: MultiWorld, player: int, target_puzzles: int):
    multiworld.completion_condition[player] = lambda state: state.has("Checkmark", player, target_puzzles)
